...
function prepareListModel(files, listOptions) {
  ...
  if (!listOptions.offset) {
    ...
    if (options.browseIdentifier === DEFAULT_PATH || globalOptions.persistImages) {
      list
    ...
    if (!globalOptions.persistImages && !globalOptions.hideThumbnails) {
      list.addListButtons([{
        title: 'Persist images',
        actionIdentifier: 'SHOW_IMAGES',
      }, {
        title: 'No thumbnails',
        actionIdentifier: 'NO_THUMBNAILS',
      }]);
    }
  ...
  itemsToAdd.map((fileInfo) => {
    ...
    if (!globalOptions.hideThumbnails) {
