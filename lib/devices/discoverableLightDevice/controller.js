...
const debug = require('debug')('neeo:discoverableLightDevice:controller');
...
module.exports.discoverProDevices = function() {
  ...
  const proDevices = sharedDeviceDiscovery()
  ...
  return BluePromise.resolve(proDevices);
};
...
setInterval(() => {
  if (sliderValue < 1) {
    debug('device powered off, don\'t send updates');
    return;
  }
  ...
  const updatePayload = {
    ...
    component: CONSTANTS.POWER_SLIDER_NAME,
    value: sliderValue
  };
  sendComponentUpdate(updatePayload)
    .catch((error) => {
     debug('failed to send slider notification', error.message);
    });
  ...
  const updateTextlabelPayload = {
   uniqueDeviceId: COMPLEX_DEVICE_UPDATE_ENTRY,
   component: CONSTANTS.TEXT_LABEL_NAME,
