...
const NOT_REACHABLE = 'LIFX_NOT_REACHABLE';
...
class LifxService {
  ...
  constructor(deviceState) {
    ...
    // the lifx library provides callback whenever a lifx lamp changes is state: discovered new lamp, lamp disappears
    this.lifxClient.on('light-new', (lifx) => {
      debug('discovered new light', lifx.id);
      // here we add a new lamp with it's id to the device state
      deviceState.addDevice(lifx.id, lifx);
    });
    this.lifxClient.on('light-online', (lifx) => {
      debug('light-online', lifx.id);
      // here we update the isReachable flag to true for a specific lamp
      deviceState.updateReachable(lifx.id, true);
    });
    this.lifxClient.on('light-offline', (lifx) => {
      debug('light-offline', lifx.id);
      // here we update the isReachable flag to false for a specific lamp
  ...
  getState(deviceId) {
    // get the state of a lamp IF the lamp is reachable and known
