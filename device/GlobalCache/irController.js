...
module.exports = class IrController {
    ...
    buildDevice() {
        let device = this._neeoapi.buildDevice('IP2IR (' + this._fileName + ')')
            ...
            .enableDiscovery({
                headerText: 'IR Settings',
                description: 'Modify the ' + this._fileName + ' file to include IR commands'
        ...
        return new BluePromise((resolve, reject) => {
            ...
            if (mapping) {
                ...
                return discovery.getDevice(device.uuid)
                    .then(results => {
                        return results.sendIR(device.module, device.port, mapping.cmd);
                    });
            } else {
    ...
    _discoverDevices() {
        return discovery
            .getDevicePorts(constants.BEACON_TYPE_IR)
            .map(port => {
